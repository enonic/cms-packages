...
<xsl:stylesheet exclude-result-prefixes="#all"
  ...
  <xsl:import href="/modules/library-stk/utilities/frame.xsl"/>
  ...
  <xsl:template match="content">
    <xsl:variable name="width">
      <xsl:choose>
        <xsl:when test="contentdata/scale = 'true' or $fw:device-class = 'mobile'">
        ...
        <p>
          <xsl:call-template name="util:image.display">
            <xsl:with-param name="filter">
              <xsl:if test="not(contentdata/scale = 'true' or $fw:device-class = 'mobile')">
                <xsl:value-of select="concat('scalewidth(', $width, ');')"/>
              </xsl:if>
              <xsl:value-of select="$fw:config-filter"/>
            </xsl:with-param>
            <xsl:with-param name="image" select="/result/contents/relatedcontents/content[@key = current()/contentdata/image/@key]"/>
            <xsl:with-param name="size">
              <xsl:if test="contentdata/scale = 'true' or $fw:device-class = 'mobile'">full</xsl:if>
            </xsl:with-param>
          </xsl:call-template>
        </p>
